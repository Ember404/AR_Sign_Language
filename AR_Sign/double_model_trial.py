...
model_point = tf.keras.models.load_model("hand_points/points.keras")
...
input_shape_point = model_point.input_shape[-1]
...
while True:
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if points_np.shape[1] == input_shape_point:
                pred = model_point.predict(points_np, verbose=0)
                class_id = np.argmax(pred)
                confidence = np.max(pred)

                # Print on frame
                cv2.putText(frame, f"Pred point: {labels[class_id]} ({confidence:.2f})",
            ...
            if angles.shape[1] == input_shape:
                ...
                # Print on frame
                cv2.putText(frame, f"Pred angle: {labels[class_id]} ({confidence:.2f})",
